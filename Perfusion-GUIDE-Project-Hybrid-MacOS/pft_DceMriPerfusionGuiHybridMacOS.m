function varargout = pft_DceMriPerfusionGuiHybridMacOS(varargin)

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @pft_DceMriPerfusionGuiHybridMacOS_OpeningFcn, ...
                   'gui_OutputFcn',  @pft_DceMriPerfusionGuiHybridMacOS_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function pft_DceMriPerfusionGuiHybridMacOS_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for pft_DceMriPerfusionGuiHybridMacOS
...
MacOSHybridMappingFolder = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Mapping');

if (exist(MacOSHybridMappingFolder, 'dir') ~= 7)
  mkdir(MacOSHybridMappingFolder);
end

ScreenshotsFolder = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Screenshots');
...
% UIWAIT makes pft_DceMriPerfusionGuiHybridMacOS wait for user response (see UIRESUME)
...
function varargout = pft_DceMriPerfusionGuiHybridMacOS_OutputFcn(hObject, eventdata, handles) 
...
Listing = dir(fullfile(handles.TargetFolder, 'MacOS-Hybrid-Screenshots', sprintf('%s_Cine-Stack_*.png', handles.FileNameStub)));
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Screenshots', sprintf('%s_Cine-Stack_001.png', handles.FileNameStub));
else
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Screenshots', sprintf('%s_Cine-Stack_%s.png', handles.FileNameStub, Suffix));
end
...
if (FilterIndex ~= 0)
  ...
  GraphFileName = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Screenshots', sprintf('%s_AIF_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.AifAxes, GraphFileName, '-png');
  
  waitbar(0.32, wb, 'Exported 2 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Screenshots', sprintf('%s_Time-Course_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.TimeCourseAxes, GraphFileName, '-png');
  
  waitbar(0.48, wb, 'Exported 3 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Screenshots', sprintf('%s_Residue-Function_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.DeconvolvedTimeCourseAxes, GraphFileName, '-png');
  
  waitbar(0.64, wb, 'Exported 4 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Screenshots', sprintf('%s_SVD_%s.png', handles.FileNameStub, Suffix));
...
Listing = dir(fullfile(handles.TargetFolder, 'MacOS-Hybrid-Mapping', sprintf('%s_HYBRID_*.csv', handles.FileNameStub)));
Entries = { Listing.name };
Folders = [ Listing.isdir ];
Entries(Folders) = [];
Entries = sort(Entries);
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Mapping', sprintf('%s_HYBRID_001.csv', handles.FileNameStub));
else
  LastName = Entries{end};
  p = strfind(LastName, '_');
  q = p(end) + 1;
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Mapping', sprintf('%s_HYBRID_%s.csv', handles.FileNameStub, Suffix));
end
...
fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
fprintf(CsvSummaryFile, '%s\n', Head{end});
fprintf(CsvSummaryFile, '%s,%s,%s,%2d,%s,%.2f,%s,%.2f\n', handles.Normalisation, handles.MatrixAlgebra, handles.Management, RSV, ZF, PC, UsePolyFit, handles.Decades); 
fprintf(CsvSummaryFile, '\n');
...
MpaRoiPathName = fullfile(handles.TargetFolder, 'MacOS-Hybrid-Mapping', MpaRoiFileName);
