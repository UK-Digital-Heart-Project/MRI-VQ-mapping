function varargout = pft_DceMriPerfusionGuiHybrid(varargin)

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @pft_DceMriPerfusionGuiHybrid_OpeningFcn, ...
                   'gui_OutputFcn',  @pft_DceMriPerfusionGuiHybrid_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function pft_DceMriPerfusionGuiHybrid_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for pft_DceMriPerfusionGuiHybrid
...
HybridMappingFolder = fullfile(handles.TargetFolder, 'Hybrid-Mapping');

if (exist(HybridMappingFolder, 'dir') ~= 7)
  mkdir(HybridMappingFolder);
end

ScreenshotsFolder = fullfile(handles.TargetFolder, 'Hybrid-Screenshots');
...
% UIWAIT makes pft_DceMriPerfusionGuiHybrid wait for user response (see UIRESUME)
...
function varargout = pft_DceMriPerfusionGuiHybrid_OutputFcn(hObject, eventdata, handles) 
...
Listing = dir(fullfile(handles.TargetFolder, 'Hybrid-Screenshots', sprintf('%s_Cine-Stack_*.png', handles.FileNameStub)));
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'Hybrid-Screenshots', sprintf('%s_Cine-Stack_001.png', handles.FileNameStub));
else
  ...
  DefaultName = fullfile(handles.TargetFolder, 'Hybrid-Screenshots', sprintf('%s_Cine-Stack_%s.png', handles.FileNameStub, Suffix));
end

FilterSpec  = fullfile(handles.TargetFolder, 'Hybrid-Screenshots', '*.png');
...
if (FilterIndex ~= 0)
  ...
  GraphFileName = fullfile(handles.TargetFolder, 'Hybrid-Screenshots', sprintf('%s_AIF_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.AifAxes, GraphFileName, '-png');
  
  waitbar(0.32, wb, 'Exported 2 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'Hybrid-Screenshots', sprintf('%s_Time-Course_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.TimeCourseAxes, GraphFileName, '-png');
  
  waitbar(0.48, wb, 'Exported 3 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'Hybrid-Screenshots', sprintf('%s_Residue-Function_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.DeconvolvedTimeCourseAxes, GraphFileName, '-png');
  
  waitbar(0.64, wb, 'Exported 4 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'Hybrid-Screenshots', sprintf('%s_SVD_%s.png', handles.FileNameStub, Suffix));
...
Listing = dir(fullfile(handles.TargetFolder, 'Hybrid-Mapping', sprintf('%s_HYBRID_*.xlsx', handles.FileNameStub)));
Entries = { Listing.name };
Folders = [ Listing.isdir ];
Entries(Folders) = [];
Entries = sort(Entries);
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'Hybrid-Mapping', sprintf('%s_HYBRID_001.xlsx', handles.FileNameStub));
else
  LastName = Entries{end};
  p = strfind(LastName, '_');
  q = p(end) + 1;
  ...
  DefaultName = fullfile(handles.TargetFolder, 'Hybrid-Mapping', sprintf('%s_HYBRID_%s.xlsx', handles.FileNameStub, Suffix));
end

FilterSpec  = fullfile(handles.TargetFolder, 'Hybrid-Mapping', '*.xlsx');
...
% Prompt for the mapping threshold
PC = handles.PC;

PC = pft_GetMappingThreshold(PC);
...
Data = { handles.Normalisation, handles.MatrixAlgebra, handles.Management, RSV, ZF, PC, UsePolyFit, handles.Decades };
Full = vertcat(Head, Data);
...
MpaRoiFileName = sprintf('%s_HYBRID-MPA-ROI_%s.png', handles.FileNameStub, Suffix);
MpaRoiPathName = fullfile(handles.TargetFolder, 'Hybrid-Mapping', MpaRoiFileName);
...
save(PicklePathName, 'AllPBV', 'AllPBF', 'AllMTT', 'AllTTP', 'UnfilteredAllPBV', 'UnfilteredAllMTT', ...
                     'ROI');
