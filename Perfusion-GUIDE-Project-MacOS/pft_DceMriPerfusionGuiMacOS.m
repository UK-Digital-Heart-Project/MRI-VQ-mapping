function varargout = pft_DceMriPerfusionGuiMacOS(varargin)

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @pft_DceMriPerfusionGuiMacOS_OpeningFcn, ...
                   'gui_OutputFcn',  @pft_DceMriPerfusionGuiMacOS_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function pft_DceMriPerfusionGuiMacOS_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for pft_DceMriPerfusionGuiMacOS
...
MacOSMappingFolder = fullfile(handles.TargetFolder, 'MacOS-Mapping');

if (exist(MacOSMappingFolder, 'dir') ~= 7)
  mkdir(MacOSMappingFolder);
end

MacOSScreenshotsFolder = fullfile(handles.TargetFolder, 'MacOS-Screenshots');
...
% UIWAIT makes pft_DceMriPerfusionGuiMacOS wait for user response (see UIRESUME)
...
function varargout = pft_DceMriPerfusionGuiMacOS_OutputFcn(hObject, eventdata, handles) 
...
Listing = dir(fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_Cine-Stack_*.png', handles.FileNameStub)));
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_Cine-Stack_001.png', handles.FileNameStub));
else
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_Cine-Stack_%s.png', handles.FileNameStub, Suffix));
end
...
if (FilterIndex ~= 0)
  ...
  GraphFileName = fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_AIF_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.AifAxes, GraphFileName, '-png');
  
  waitbar(0.32, wb, 'Exported 2 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_Time-Course_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.TimeCourseAxes, GraphFileName, '-png');
  
  waitbar(0.48, wb, 'Exported 3 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_Residue-Function_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.DeconvolvedTimeCourseAxes, GraphFileName, '-png');
  
  waitbar(0.64, wb, 'Exported 4 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_SVD_%s.png', handles.FileNameStub, Suffix));
...
% First, offer to save the processing parameters in a CSV file
Listing = dir(fullfile(handles.TargetFolder, 'MacOS-Mapping', sprintf('%s_MAPPING_*.csv', handles.FileNameStub)));
Entries = { Listing.name };
Folders = [ Listing.isdir ];
Entries(Folders) = [];
Entries = sort(Entries);
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Mapping', sprintf('%s_MAPPING_001.csv', handles.FileNameStub));
else
  LastName = Entries{end};
  p = strfind(LastName, '_');
  q = p(end) + 1;
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Mapping', sprintf('%s_MAPPING_%s.csv', handles.FileNameStub, Suffix));
end

FilterSpec  = fullfile(handles.TargetFolder, 'MacOS-Mapping', '*.csv');
...
fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
fprintf(CsvSummaryFile, '%s\n', Head{end});
fprintf(CsvSummaryFile, '%s,%s,%s,%2d,%s,%.2f,%.2f\n', handles.Normalisation, handles.MatrixAlgebra, handles.Management, RSV, ZF, PC, handles.Decades); 
fprintf(CsvSummaryFile, '\n');
...
MpaRoiPathName = fullfile(handles.TargetFolder, 'MacOS-Mapping', MpaRoiFileName);
