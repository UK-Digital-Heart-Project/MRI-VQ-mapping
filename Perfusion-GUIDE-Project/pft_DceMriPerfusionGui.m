function varargout = pft_DceMriPerfusionGui(varargin)

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @pft_DceMriPerfusionGui_OpeningFcn, ...
                   'gui_OutputFcn',  @pft_DceMriPerfusionGui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function pft_DceMriPerfusionGui_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for pft_DceMriPerfusionGui
...
MappingFolder = fullfile(handles.TargetFolder, 'Mapping');

if (exist(MappingFolder, 'dir') ~= 7)
  mkdir(MappingFolder);
end
...
% UIWAIT makes pft_DceMriPerfusionGui wait for user response (see UIRESUME)
...
function varargout = pft_DceMriPerfusionGui_OutputFcn(hObject, eventdata, handles) 
...
Listing = dir(fullfile(handles.TargetFolder, 'Screenshots', sprintf('%s_Cine-Stack_*.png', handles.FileNameStub)));
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'Screenshots', sprintf('%s_Cine-Stack_001.png', handles.FileNameStub));
else
  ...
  DefaultName = fullfile(handles.TargetFolder, 'Screenshots', sprintf('%s_Cine-Stack_%s.png', handles.FileNameStub, Suffix));
end
...
if (FilterIndex ~= 0)
  ...
  GraphFileName = fullfile(handles.TargetFolder, 'Screenshots', sprintf('%s_AIF_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.AifAxes, GraphFileName, '-png');
  
  waitbar(0.32, wb, 'Exported 2 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'Screenshots', sprintf('%s_Time-Course_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.TimeCourseAxes, GraphFileName, '-png');
  
  waitbar(0.48, wb, 'Exported 3 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'Screenshots', sprintf('%s_Residue-Function_%s.png', handles.FileNameStub, Suffix));
  export_fig(handles.DeconvolvedTimeCourseAxes, GraphFileName, '-png');
  
  waitbar(0.64, wb, 'Exported 4 of 5 axes ... ');

  GraphFileName = fullfile(handles.TargetFolder, 'Screenshots', sprintf('%s_SVD_%s.png', handles.FileNameStub, Suffix));
...
Listing = dir(fullfile(handles.TargetFolder, 'Mapping', sprintf('%s_MAPPING_*.xlsx', handles.FileNameStub)));
Entries = { Listing.name };
Folders = [ Listing.isdir ];
Entries(Folders) = [];
Entries = sort(Entries);
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'Mapping', sprintf('%s_MAPPING_001.xlsx', handles.FileNameStub));
else
  LastName = Entries{end};
  p = strfind(LastName, '_');
  q = p(end) + 1;
  ...
  DefaultName = fullfile(handles.TargetFolder, 'Mapping', sprintf('%s_MAPPING_%s.xlsx', handles.FileNameStub, Suffix));
end

FilterSpec  = fullfile(handles.TargetFolder, 'Mapping', '*.xlsx');
...
% Prompt for a processing lower threshold
PC = pft_GetMappingThreshold;
...
Data = num2cell([handles.RoiSlice, handles.Reduction, handles.LastUsableFrame]);
Full = vertcat(Head, Data);
  
xlswrite(fullfile(PathName, FileName), Full, 'ROI');

waitbar(0.2, wb, 'Saved sheet 1 of 4 ... ');
  
Head = { 'Epoch', 'Acquisition time /sec', 'Mean AIF', 'Peak AIF' };
Data = num2cell([(1:handles.NEPOCHS)', handles.Mat.AT, handles.AIFMean, handles.AIFPeak]);
Full = vertcat(Head, Data);
  
xlswrite(fullfile(PathName, FileName), Full, 'AIF'); 

waitbar(0.4, wb, 'Saved sheet 2 of 4 ... ');
  
Head = { 'Normalisation', 'Matrix Algebra', 'SVD Management', 'Retained Singular Values', 'Zero-Fill Truncated AIF', 'Lower Threshold: PC of Maximum AIF', 'Gaussian filter (decades)' };
...
Data = { handles.Normalisation, handles.MatrixAlgebra, handles.Management, RSV, ZF, PC, handles.Decades };
Full = vertcat(Head, Data);
  
xlswrite(fullfile(PathName, FileName), Full, 'Deconvolution');

waitbar(0.6, wb, 'Saved sheet 3 of 4 ... ');

Head = { 'Epoch', 'Acquisition time / sec', 'Filter' };
Data = num2cell([(2:handles.LastUsableFrame)', handles.Mat.AT(2:handles.LastUsableFrame), handles.Filter]);
Full = vertcat(Head, Data);

xlswrite(fullfile(PathName, FileName), Full, 'Filtering');

waitbar(0.8, wb, 'Saved sheet 4 of 4 ... ');
...
MpaRoiFileName = sprintf('%s_MPA-ROI_%s.png', handles.FileNameStub, Suffix);
MpaRoiPathName = fullfile(handles.TargetFolder, 'Mapping', MpaRoiFileName);
...
mkdir(Container, 'Unfiltered-PBF');
...
UnfilteredAllPBF = zeros([handles.NROWS, handles.NCOLS, handles.NSLICES], 'uint16');
...
for s = 1:handles.NSLICES
  ...
  UnfilteredPBF = zeros([handles.NROWS, handles.NCOLS], 'double');
  ...
  for c = 1:handles.NCOLS
    ...
    for r = 1:handles.NROWS
      ...
      if ~isempty(Residue)
        ResidueIntegral = 0.5*(Residue(1) + Residue(M)) + sum(Residue(2:end-1));
        
        [ Value, Place ] = max(Residue(1:M));
        
        MTT(r, c) = DT*ResidueIntegral/Value;
        
        PBF(r, c) = PBV(r, c)/MTT(r, c);
        
        TTP(r, c) = Times(Place);
        
        UnfilteredPBF(r, c) = UnfilteredPBV(r, c)/MTT(r, c);
      end
  ...
  UnfilteredAllPBF(:, :, s) = UnfilteredPBF;
  ...
  Info.SeriesDescription = 'RSS parameter map';
  Info.ImageComments     = 'Unfiltered pulmonary blood flow';
  Info.RescaleIntercept  = 0.0;
  Info.RescaleSlope      = 1.0;
  Info.RescaleType       = '(ml/min)/100.0 ml';
  
  UnfilteredPbfFileName = fullfile(Container, 'Unfiltered-PBF', sprintf('Unfiltered-PBF-%04d.dcm', s));
  dicomwrite(UnfilteredPBF, UnfilteredPbfFileName, Info, 'Dictionary', Dictionary, 'CreateMode', 'copy', 'WritePrivate', true, 'UseMetadataBitDepths', true);
...
save(PicklePathName, 'AllPBV', 'AllPBF', 'AllMTT', 'AllTTP', 'UnfilteredAllPBV', 'UnfilteredAllPBF', 'ROI');
