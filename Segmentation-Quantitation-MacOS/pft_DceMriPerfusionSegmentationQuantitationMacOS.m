function varargout = pft_DceMriPerfusionSegmentationQuantitationMacOS(varargin)

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @pft_DceMriPerfusionSegmentationQuantitationMacOS_OpeningFcn, ...
                   'gui_OutputFcn',  @pft_DceMriPerfusionSegmentationQuantitationMacOS_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function pft_DceMriPerfusionSegmentationQuantitationMacOS_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for pft_DceMriPerfusionSegmentationQuantitationMacOS
...
QuantitationFolder = fullfile(handles.TargetFolder, 'MacOS-Quantitation');

if (exist(QuantitationFolder, 'dir') ~= 7)
  mkdir(QuantitationFolder);
end

ScreenshotsFolder = fullfile(handles.TargetFolder, 'MacOS-Screenshots');

if (exist(ScreenshotsFolder, 'dir') ~= 7)
  mkdir(ScreenshotsFolder);
end

MoviesFolder = fullfile(handles.TargetFolder, 'MacOS-Movies');
...
% UIWAIT makes pft_DceMriPerfusionSegmentationQuantitationMacOS wait for user response (see UIRESUME)
...
function varargout = pft_DceMriPerfusionSegmentationQuantitationMacOS_OutputFcn(hObject, eventdata, handles) 
...
Listing = dir(fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_Screenshot_*.png', handles.FileNameStub)));
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_Screenshot_001.png', handles.FileNameStub));
else
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Screenshots', sprintf('%s_Screenshot_%s.png', handles.FileNameStub, Suffix));
end
...
Listing = dir(fullfile(handles.TargetFolder, 'MacOS-Quantitation', sprintf('%s_QUANTIFICATION_*.csv', handles.FileNameStub)));
Entries = { Listing.name };
Folders = [ Listing.isdir ];
Entries(Folders) = [];
Entries = sort(Entries);
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Quantitation', sprintf('%s_QUANTIFICATION_001.csv', handles.FileNameStub));
else
  LastName = Entries{end};
  p = strfind(LastName, '_');
  q = p(end) + 1;
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Quantitation', sprintf('%s_QUANTIFICATION_%s.csv', handles.FileNameStub, Suffix));
end
...
CsvSummaryFileName = fullfile(PathName, FileName);

CsvSummaryFile = fopen(CsvSummaryFileName, 'wt');

set(handles.CsvSummaryFileEdit, 'String', sprintf('  CSV Summary File:      %s', FileName));
...
fprintf(CsvSummaryFile, 'Resolution\n\n');
fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
fprintf(CsvSummaryFile, '%s\n', Head{end});
fprintf(CsvSummaryFile, '%3d,%3d,%3d,%.4f,%.4f,%.4f\n', Data');
fprintf(CsvSummaryFile, '\n');

waitbar(double(1)/double(NTABS + 1), wb, sprintf('Saved 1 out of %1d sections', NTABS));
...
fprintf(CsvSummaryFile, 'Deficits\n\n');
fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
fprintf(CsvSummaryFile, '%s\n', Head{end});
fprintf(CsvSummaryFile, '%d,%.4f,%d,%.4f,%.4f,%d,%.4f,%d,%.4f,%.4f,%d,%.4f,%d,%.4f,%.4f\n', Data');
fprintf(CsvSummaryFile, '\n');

waitbar(double(2)/double(NTABS + 1), wb, sprintf('Saved 2 out of %1d sections', NTABS));
...
fprintf(CsvSummaryFile, 'PBV\n\n');
fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
fprintf(CsvSummaryFile, '%s\n', Head{end});
fprintf(CsvSummaryFile, '%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n', Data');
fprintf(CsvSummaryFile, '\n');

waitbar(double(3)/double(NTABS + 1), wb, sprintf('Saved 3 out of %1d sections', NTABS));
...
fprintf(CsvSummaryFile, 'Unfiltered PBV\n\n');
fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
fprintf(CsvSummaryFile, '%s\n', Head{end});
fprintf(CsvSummaryFile, '%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n', Data');
fprintf(CsvSummaryFile, '\n');

waitbar(double(4)/double(NTABS + 1), wb, sprintf('Saved 4 out of %1d sections', NTABS));
...
fprintf(CsvSummaryFile, 'PBF\n\n');
fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
fprintf(CsvSummaryFile, '%s\n', Head{end});
fprintf(CsvSummaryFile, '%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n', Data');
fprintf(CsvSummaryFile, '\n');
      
waitbar(double(5)/double(NTABS + 1), wb, sprintf('Saved 5 out of %1d sections', NTABS));
...
if ~isempty(handles.UnfilteredPBF)
  ...
  fprintf(CsvSummaryFile, 'Unfiltered PBF\n\n');
  fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
  fprintf(CsvSummaryFile, '%s\n', Head{end});
  fprintf(CsvSummaryFile, '%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n', Data');
  fprintf(CsvSummaryFile, '\n');        
...
waitbar(double(6)/double(NTABS + 1), wb, sprintf('Saved 6 out of %1d sections', NTABS));
...
fprintf(CsvSummaryFile, 'MTT\n\n');
fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
fprintf(CsvSummaryFile, '%s\n', Head{end});
fprintf(CsvSummaryFile, '%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n', Data');
fprintf(CsvSummaryFile, '\n');       

waitbar(double(7)/double(NTABS + 1), wb, sprintf('Saved 7 out of %1d sections', NTABS));
...
if ~isempty(handles.UnfilteredMTT)
  ...
  fprintf(CsvSummaryFile, 'Unfiltered MTT\n\n');
  fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
  fprintf(CsvSummaryFile, '%s\n', Head{end});
  fprintf(CsvSummaryFile, '%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n', Data');
  fprintf(CsvSummaryFile, '\n');   
...
waitbar(double(8)/double(NTABS + 1), wb, sprintf('Saved 8 out of %1d sections', NTABS));
...
fprintf(CsvSummaryFile, 'TTP\n\n');
fprintf(CsvSummaryFile, '%s,', Head{1:end-1});
fprintf(CsvSummaryFile, '%s\n', Head{end});
fprintf(CsvSummaryFile, '%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n', Data');
fprintf(CsvSummaryFile, '\n');   

waitbar(double(9)/double(NTABS + 1), wb, sprintf('Saved 9 out of %1d sections', NTABS));
...
fprintf(CsvSummaryFile, 'Censorship\n\n');
fprintf(CsvSummaryFile, '%s\n', Head{1});
fprintf(CsvSummaryFile, '%s\n', Data{1});

fclose(CsvSummaryFile);

waitbar(double(10)/double(NTABS + 1), wb, sprintf('Saved 10 out of %1d sections', NTABS));

% Quit gracefully
pause(0.5);
waitbar(1, wb, 'All sections saved !');
pause(0.5);
delete(wb);
...
function CsvSummaryFileEdit_Callback(hObject, eventdata, handles)
  % Nothing to do here - this edit window is read-only (for the time being)
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function CsvSummaryFileEdit_CreateFcn(hObject, eventdata, handles)
...
% Offer the option to save the screenshot as an image
Listing = dir(fullfile(handles.TargetFolder, 'MacOS-Movies', sprintf('%s_Movie_*.avi', handles.FileNameStub)));
Entries = { Listing.name };
Folders = [ Listing.isdir ];
Entries(Folders) = [];
Entries = sort(Entries);
...
if isempty(Entries)
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Movies', sprintf('%s_Movie_001.avi', handles.FileNameStub));
else
  LastName = Entries{end};
  p = strfind(LastName, '_');
  q = p(end) + 1;
  ...
  DefaultName = fullfile(handles.TargetFolder, 'MacOS-Movies', sprintf('%s_Movie_%s.avi', handles.FileNameStub, Suffix));
end
