...
Fptr = fopen('Hybrid-Mapping-Folder.txt', 'rt');
Root = fscanf(Fptr, '%s');
fclose(Fptr);

%% List all the summary files present

Listing = dir(fullfile(Root, 'HH*HYBRID*.xlsx'));
...
Target = fullfile(Root, 'HYBRID-MAPPING-COMPILATION.xlsx');
Backup = fullfile(Root, 'HYBRID-MAPPING-COMPILATION-BACKUP.xlsx');
...
for n = 1:NFILES
  ...
  pft_AppendOneXlsxHybridMappingFile(Source, Target);
