...
Fptr = fopen('Ingrisch-Mapping-Folder.txt', 'rt');
Root = fscanf(Fptr, '%s');
fclose(Fptr);

%% List all the summary files present

Listing = dir(fullfile(Root, 'HH*INGRISCH*.xlsx'));
...
Target = fullfile(Root, 'INGRISCH-MAPPING-COMPILATION.xlsx');
Backup = fullfile(Root, 'INGRISCH-MAPPING-COMPILATION-BACKUP.xlsx');
...
for n = 1:NFILES
  ...
  pft_AppendOneXlsxIngrischMappingFile(Source, Target);
